...
export default function SupportPage() {
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold mb-4">Support & Help Center</h1>
      <div className="mb-8">
        <h2 className="font-semibold mb-2">Frequently Asked Questions</h2>
        <ul className="space-y-2">
          {faqs.map((faq, idx) => (
            <li key={idx} className="bg-white rounded shadow p-4">
              <button className="w-full text-left font-semibold" onClick={() => setOpen(open === idx ? null : idx)}>{faq.q}</button>
              {open === idx && <div className="mt-2 text-gray-700">{faq.a}</div>}
            </li>
          ))}
        </ul>
      </div>
      <form className="bg-white rounded shadow p-4 max-w-lg">
        <label className="block mb-2 font-semibold">Contact Support</label>
