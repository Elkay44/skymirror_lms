...
export default function ActivityFeed({
  ...
  return (
    <div className={`bg-white rounded-xl shadow-sm p-6 ${className}`}>
      <h3 className="text-lg font-semibold text-gray-900 mb-6 break-words">{title}</h3>
      ...
      {displayActivities.length === 0 ? (
        ...
        <div className="space-y-4 lg:space-y-6">
          {displayActivities.map((activity) => (
            <div key={activity.id} className="flex items-start min-w-0">
              {/* Activity icon */}
              ...
              <div className="flex-1 min-w-0 min-w-0">
                <div className="flex justify-between min-w-0">
                  ...
                  <span className="text-sm text-gray-500 break-words">{formatRelativeTime(activity.timestamp)}</span>
                </div>
                
                {activity.description && (
                  <p className="mt-1 text-sm text-gray-500 line-clamp-2 break-words">{activity.description}</p>
                )}
                
                {/* Related item (course, achievement, etc.) */}
                {activity.relatedItem && (
                  <div className="mt-2 flex items-center bg-gray-50 rounded-lg p-2 min-w-0">
                    {activity.relatedItem.image && (
                      <div className="flex-shrink-0 h-10 w-10 mr-3 relative rounded overflow-hidden min-w-0">
                        <Image 
                    ...
                    <div className="min-w-0 flex-1 min-w-0">
                      <p className="text-xs font-medium text-gray-900 truncate break-words">{activity.relatedItem.title}</p>
                    </div>
                  </div>
                )}
                ...
                {activity.link && (
                  <div className="mt-2">
                    <Link 
                      href={activity.link}
                      className="text-sm font-medium text-indigo-600 hover:text-indigo-800 break-words"
                    >
                      View details
                    </Link>
                  </div>
