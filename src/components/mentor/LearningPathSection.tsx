    ...
    <div className="space-y-4 lg:space-y-6">
        ...
        <div className="p-4 bg-teal-50 border-b border-teal-100">
          <div className="flex items-center min-w-0">
            <div className="flex-shrink-0 mr-3 min-w-0">
              <div className="h-12 w-12 rounded-full bg-teal-100 flex items-center justify-center min-w-0">
                <Target className="h-6 w-6 text-teal-600" />
              </div>
            </div>
            <div>
              <h3 className="text-lg font-medium text-gray-900 break-words">{learningPath} Path</h3>
        ...
        <div className="p-4">
          <div className="mb-6">
            <div className="flex justify-between items-center mb-2 min-w-0">
              <h4 className="text-sm font-medium text-gray-700 break-words">Overall Path Progress</h4>
              <span className="text-sm font-medium text-teal-600 break-words">
                {skills.length > 0 ? 
            ...
            <div className="bg-purple-50 border border-purple-100 rounded-lg p-4">
              <div className="flex items-center min-w-0">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900 break-words">{skills.length}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {Object.keys(skillsByCategory).length > 0 ? (
            <div className="space-y-6 lg:space-y-8">
              {Object.entries(skillsByCategory).map(([category, categorySkills]) => (
                <div key={`category-${category}`} className="space-y-4">
                  <h4 className="text-md font-medium text-gray-900 border-b border-gray-200 pb-2 break-words">{category}</h4>
                      ...
                      <div key={skill.id} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between min-w-0">
                          <div className="mb-3 sm:mb-0">
                            <h5 className="text-sm font-medium text-gray-900 break-words">{skill.name}</h5>
                          ...
                          <div className="flex items-center min-w-0">
                            ...
                            <span className="ml-2 text-xs font-medium text-gray-700 break-words">{skill.proficiency}%</span>
                          </div>
                        </div>
                        
                        {skill.recommendation && (
                          <div className="mt-3 bg-white border border-gray-200 rounded-md p-3 text-xs text-gray-700 overflow-hidden">
                            <div className="flex min-w-0">
                              <Info className="h-4 w-4 text-teal-500 flex-shrink-0 mr-2 mt-0.5 min-w-0" />
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-1 break-words">No Skills Assessment</h3>
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden overflow-hidden">
        <div className="p-4 border-b border-gray-200 bg-gray-50">
          <h3 className="text-md font-medium text-gray-900 break-words">Learning Recommendations</h3>
        </div>
        ...
        <div className="p-4">
          <div className="space-y-4">
            {skills.some(skill => skill.proficiency < 50) ? (
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-3 break-words">Focus Areas</h4>
                <div className="space-y-3">
                  {skills
                    ...
                    .slice(0, 3)
                    .map(skill => (
                      <div key={`focus-${skill.id}`} className="flex items-start p-3 bg-yellow-50 border border-yellow-100 rounded-lg min-w-0">
                        <AlertTriangle className="h-5 w-5 text-yellow-500 mr-3 flex-shrink-0 mt-0.5 min-w-0" />
                        <div>
                          <p className="text-sm font-medium text-gray-900 break-words">{skill.name} ({skill.proficiency}%)</p>
            ...
            <div className="flex justify-end mt-4 min-w-0">
              <button
